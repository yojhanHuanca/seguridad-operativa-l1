...
function fechaDDMMAAAA(iso: string) {
...
export function filaCsv(e: EventoListItem) {
  return [
    fechaDDMMAAAA(e.fecha),
    e.hora ? formatTime(e.hora) : "",
    e.anio,
...
export function coincideBusqueda(evento: EventoListItem, query: string) {
  ...
  return (
    ...
    (evento.codigo_evento ?? "").toLowerCase().includes(q)
  );
}
 ...
 */
export const COLUMNAS_EVENTO: { header: string; render: (e: EventoListItem) => ReactNode; nowrap?: boolean }[] = [
  { header: "Fecha", render: (e) => formatDate(e.fecha), nowrap: true },
  { header: "Hora", render: (e) => (e.hora ? formatTime(e.hora) : "—"), nowrap: true },
  ...
  { header: "Tipo de incidente", render: (e) => e.catalogo_detalle_eventos_monitoreo_tipo_incidenteTocatalogo_detalle?.nombre ?? "—", nowrap: true },
  ...
  { header: "Lugar", render: (e) => e.catalogo_detalle_eventos_monitoreo_lugar_incidenteTocatalogo_detalle?.nombre ?? "—", nowrap: true },
  ...
  { header: "Personal/Falla", render: (e) => e.catalogo_detalle_eventos_monitoreo_personal_involucradoTocatalogo_detalle?.nombre ?? "—", nowrap: true },
  ...
  { header: "Info adicional", render: (e) => e.informacion_adicional || "—" },
  { header: "Cámara", render: (e) => e.camara_monitoreada ?? "—", nowrap: true },
