...
import {
  ...
  VencimientoCard,
  type MonthTrendItem,
} from "@/features/indicadores/components/IndicadoresPrintDocument";
...
export function SoKpisPage() {
  ...
  const { printActive, printReport } = useIndicadoresPrint(`Indicadores ${systemName}`);
    ...
    <SeguridadOperativaShell>
      <div data-report-export-root={printActive ? "active" : "idle"}>
        {printActive && (
          <IndicadoresPrintDocument
            ...
            panelLabel="Seguridad Operativa"
            titulo="Indicadores | Gestiona+"
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        <ReportesSopCard
          ...
          cerradoVsProceso={cerradoVsProceso}
          isLoading={isLoading}
          activeReportType={selection?.kind === "reportType" ? selection.value : null}
          activeReportStatus={activeFor("reportStatus")}
          onSelectType={(name) => selectItem("reportType", name, "Tipo de reporte")}
          onSelectStatus={(label) => selectItem("reportStatus", label, "Estado de reporte")}
        />

        <TotalPlanesCard
          subtitle={planesSubtitle}
          planes={planes}
          isLoading={isLoading}
          activePlanStatus={selection?.kind === "planStatus" ? selection.value : null}
          onSelectPlanStatus={(label) => selectItem("planStatus", label, "Estado de plan")}
        />

        <RiesgoCard
          subtitle={riesgoSubtitle}
          riesgo={riesgo}
          riesgoTotal={riesgoTotal}
          isLoading={isLoading}
          activeRisk={selection?.kind === "risk" ? selection.value : null}
          onSelectRisk={(label) => selectItem("risk", label, "Análisis de riesgo")}
        />
      </div>
      ...
      <div className="mt-5 grid gap-5 md:grid-cols-3">
        <AreaCard
          subtitle={planesAreaSubtitle}
          porArea={porArea}
          isLoading={isLoading}
          activeArea={activeFor("planArea")}
          onSelectArea={(label) => selectItem("planArea", label, "Planes abiertos por área")}
        />

        <VencimientoCard
          subtitle={vencimientoSubtitle}
          vencimiento={vencimiento}
          isLoading={isLoading}
          activeDue={activeFor("due")}
          onSelectDue={(label) => selectItem("due", label, "Vencimiento de planes abiertos")}
        />

        <ReprogramacionCard
          subtitle={reprogramacionSubtitle}
          reprogramacion={reprogramacion}
          isLoading={isLoading}
          activeReschedule={activeFor("reschedule")}
          onSelectReschedule={(label) => selectItem("reschedule", label, "Reprogramación de planes abiertos")}
        />
      </div>
...
/**
 * Documento para `window.print()` — mismo patrón que `ReportExportPrintDocument`
 * de ReportExportPage.tsx. Usa las mismas 7 tarjetas que se ven en pantalla
 * (mismos componentes, mismo estilo, sin interactividad), solo que en el
 * orden que pidió el cliente en vez del orden de pantalla: Total de Reportes
 * SOP, Total de Planes, Reportes por Mes, Análisis de Riesgo, Planes por
 * Área, Vencimiento, Reprogramación.
 */
...
function buildDetailRows(cases: CaseListItem[], selection: DashboardSelection | null): DetailRow[] {
  ...
  cases.forEach((caso) => {
    ...
    if (selection) {
      // Selección a nivel de reporte (tipo, estado, mes, riesgo): el KPI
      // cuenta casos, así que acá va una fila por caso — no una por cada
      // plan — o el total de la tabla no coincide con el del KPI. La
      // columna de plan muestra el primero que no esté cerrado (un caso
      // "En Proceso" puede tener un plan viejo ya cerrado y otro activo;
      // mostrar ese primero confundía, como si un reporte cerrado se
      // hubiera colado en el filtro).
      const planRepresentativo =
        caso.planes_accion.find((plan) => estadoPlanIndicador(plan, caso) !== "Cerrado") ?? caso.planes_accion[0] ?? null;
      rows.push(detailRow(caso, planRepresentativo));
      return;
    }
