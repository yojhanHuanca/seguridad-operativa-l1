...
type StatusFilter = "todos" | "pendientes" | "ejecucion" | "verificacion";
...
export function JefeHome() {
  ...
  return (
    <JefeShell>
      {isLoading ? (
        <Card className="py-16 text-center text-[13px] text-ink-quiet">Cargando planes...</Card>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h1 className="text-[20px] font-bold tracking-tight text-ink">Mis Planes de Acción</h1>
              <p className="mt-1 text-[12px] text-ink-soft">Planes asignados por Seguridad Operativa</p>
            </div>
            <span className="text-[11px] text-ink-quiet">{countLabel(filteredRows.length, "caso", "casos")}</span>
          </div>
              ...
              <table className="w-full min-w-[920px] text-sm">
                <thead>
                  <tr className="border-b border-line-soft bg-surface-2">
                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-ink-quiet">Código</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-ink-quiet">Título</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-ink-quiet">Estado</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-ink-quiet">Prioridad</th>
                    <th className="px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-ink-quiet whitespace-nowrap">Fecha límite</th>
                    <th className="px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-wide text-ink-quiet">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                      ...
                      <tr key={row.codigo} className="border-b border-line-soft transition-colors last:border-b-0 hover:bg-surface-2">
                        <td className="px-4 py-3">
                          <Link
                            ...
                            className="whitespace-nowrap font-mono text-[12px] font-semibold text-brand-700 transition-colors hover:text-brand-800 hover:underline"
                          >
                            {row.codigo}
                          </Link>
                        </td>
                        <td className="px-4 py-3">
                          <span className="line-clamp-1 text-[12px] font-medium text-ink">{row.titulo}</span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={cn("whitespace-nowrap text-[12px]", overdue ? "font-semibold text-critical" : "text-ink")}>{formatDate(deadline)}</span>
                        </td>
                        <td className="px-4 py-3 text-right">
                          <Link
                            to={`/jefe/planes/${encodeURIComponent(row.codigo)}`}
                            className="whitespace-nowrap text-[11px] text-ink-quiet transition-colors hover:text-brand-700 hover:underline"
                          >
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <Card className="p-6 text-center">
        ...
        <h3 className="mb-2 text-[16px] font-semibold text-ink">Sin planes activos</h3>
        <p className="text-[13px] text-ink-soft">Cuando Seguridad Operativa asigne un Plan de Acción a su área, aparecerá aquí para su ejecución.</p>
      </Card>
      <Card className="mt-6 p-5">
        <p className="mb-3 text-[11px] font-semibold uppercase tracking-wide text-ink-faint">¿Qué puede hacer aquí?</p>
