...
function correoDelBody(req: Request): string {
  const correo = (req.body as { correo?: unknown } | undefined)?.correo;
...
export const passwordResetPorCorreoRateLimit = rateLimit({
  ...
  skip: (req) => !correoDelBody(req as Request),
  keyGenerator: (req) => correoDelBody(req as Request),
  handler: (_req, res) => {
