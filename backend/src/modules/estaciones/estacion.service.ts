...
export class EstacionService {
  ...
  static async createEstacion(nombre_estacion: string) {
    const nombre = nombre_estacion.trim();
    if (!nombre) throw new Error("El nombre de la estación es obligatorio");

    const existing = await EstacionRepository.findByNombre(nombre);
    if (existing) throw new Error("Ya existe una estación con ese nombre");

    return EstacionRepository.create(nombre);
  }

  static async updateEstacion(id_estacion: number, nombre_estacion: string) {
    const nombre = nombre_estacion.trim();
    if (!nombre) throw new Error("El nombre de la estación es obligatorio");
  ...
  static async deleteEstacion(id_estacion: number) {
