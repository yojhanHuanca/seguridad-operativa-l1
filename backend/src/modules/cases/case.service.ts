...
async function validarResponsablesJefeDeArea(planes: CreatePlanDto[]) {
  const ids = Array.from(new Set(
    planes.flatMap((plan) => [
      plan.responsable,
      ...plan.actividades.map((actividad) => actividad.responsable).filter((id): id is number => id != null),
    ])
  ));
 ...
 */
async function areaEfectiva(areaQuery: string | undefined, actor?: Actor): Promise<number | undefined> {
  if (esJefeDeArea(actor)) return areaDelActor(actor);
  const area = areaQuery ? Number(areaQuery) : undefined;
  return Number.isFinite(area) ? area : undefined;
}

async function getCasoBasico(codigo: string, accion?: AccionCaso) {
 ...
 */
async function getPlanBasico(idPlan: string | number, accion?: AccionCaso, actor?: Actor) {
  const id = Number(idPlan);
  if (!Number.isFinite(id)) throw new Error("Identificador de plan inválido");

  const plan = await CaseRepository.findPlanContexto(id);
  if (!plan) throw new Error(`El plan ${idPlan} no existe`);
  ...
  if (actor) await assertPlanPropio(plan, actor);
  if (accion) {
 ...
 * todos los planes.
 */
async function assertPlanPropio(plan: { id_area: number | null; responsable: number | null }, actor: Actor) {
  if (!esJefeDeArea(actor)) return;

  if (plan.responsable === actor.id_usuario) return;

  const areaActor = await areaDelActor(actor);
  if (plan.id_area != null && areaActor != null && plan.id_area === areaActor) return;
...
export class CaseService {
   ...
   */
  static async updateTipo(codigo: string, rawBody: unknown, actor?: Actor) {
  ...
  static async completeExecutionByPlan(idPlan: string, rawBody: unknown, actor?: Actor) {
  ...
  static async updateActivity(idActividad: string, rawBody: unknown, actor?: Actor) {
    ...
    if (!plan) throw new Error(`La actividad ${idActividad} no existe`);
    if (actor) await assertPlanPropio(plan, actor);
  ...
  static async removePlanEvidence(idPlan: string, idAnexo: string, _rawBody: unknown, actor?: Actor) {
    const plan = await getPlanBasico(idPlan, undefined, actor);
  ...
  static async addPlanUpdate(idPlan: string, rawBody: unknown, files: UploadedFile[], actor?: Actor) {
  ...
  static async addEvidenceByPlan(idPlan: string, _rawBody: unknown, files: UploadedFile[], actor?: Actor) {
    const plan = await getPlanBasico(idPlan, undefined, actor);
