...
const normalizarRol = (rol?: string | null) => (rol ?? "").trim().toLowerCase();
const puedeRegistrarComoSO = (actor?: Actor) => {
...
export class ReportService {
  static async createReport(rawBody: unknown, files: UploadedFile[], actor?: Actor) {
  ...
  static async getByCodigo(codigo_sop: string, actor?: Actor) {
    if (!actor) {
      ...
      if (!casoPublico) throw new Error(`El caso ${codigo_sop} no existe`);
      return casoPublico;
    }
