...
function verificarContenidoReal(obtenerArchivos) {
    return (req, res, next) => {
        ...
        for (const archivo of archivos) {
            ...
            if (!esValido) {
                ...
                return res.status(400).json(ApiResponse.error(`El archivo "${archivo.originalname}" no corresponde a su tipo declarado (${archivo.mimetype})`));
            }
        }
        next();
    };
...
export const verificarContenidoEvidencia = verificarContenidoReal((req) => req.files ?? []);
