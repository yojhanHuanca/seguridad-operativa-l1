...
export function tieneRol(actor, rol) {
    return normalizar(actor?.rol_nombre) === normalizar(rol);
}
export const esReportante = (actor) => tieneRol(actor, "Reportante");
export const esJefeDeArea = (actor) => tieneRol(actor, "Jefe de Área");
export const esAdmin = (actor) => tieneRol(actor, "Admin");
/**
 ...
 */
export async function nombreDelActor(actor) {
    if (!actor)
        return "Sistema";
 ...
 */
export async function areaDelActor(actor) {
    if (!actor)
        return undefined;
    if (actor.id_area != null)
        return actor.id_area;
    const usuario = await prisma.usuarios.findUnique({
        where: { id_usuario: actor.id_usuario },
        select: { id_area: true },
    });
