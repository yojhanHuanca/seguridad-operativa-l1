...
const RISK_ORDER = ["inaceptable", "no_deseable", "aceptable_revision", "aceptable_sin_revision"];
const RISK_LABEL = {
...
function planBucket(estadoPlan, estadoCaso) {
    const p = estadoPlan.toLowerCase();
    const c = estadoCaso.toLowerCase();
    if (p.includes("rechaz") || p.includes("cerrad") || c.includes("cierre") || c === "cerrado")
        return "cerrado";
    if (p.includes("finaliz") || c.includes("verificacion") || c === "verificación")
        return "enValidacion";
    return "enProceso";
}
...
export class DashboardService {
    static async getIndicadores() {
        ...
        for (const caso of casos) {
            ...
            if (estadoStatus === "cerrado")
                cerrado++;
            else if (estadoStatus === "abierto")
                enProceso++;
        ...
        const ordenarDescendente = (mapa, color) => Array.from(mapa.entries())
