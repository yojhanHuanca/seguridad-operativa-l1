...
function nextPlanCodes(codigoSop, prefix, cantidad, codigosExistentes) {
    const secuenciasOcupadas = new Set();
    ...
    while (codigos.length < cantidad) {
        ...
        if (secuenciasOcupadas.has(sequence) || codigosOcupados.has(candidate.toUpperCase()))
            continue;
        secuenciasOcupadas.add(sequence);
