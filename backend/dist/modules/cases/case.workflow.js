 ...
 */
export function assertTransicion(accion, estadoActual) {
    if (!estadoActual)
        return;
    const permitidos = ORIGENES_VALIDOS[accion];
    if (permitidos.includes(estadoActual))
        return;
    throw new TransicionInvalidaError(`No se puede ${NOMBRE_ACCION[accion]}: el caso está en "${estadoActual}" y esta acción solo aplica en ${permitidos
        .map((e) => `"${e}"`)
        .join(", ")}.`);
}
//# sourceMappingURL=case.workflow.js.map