...
function extractRiesgoCode(value) {
...
const TIPO_HALLAZGO_MAP = {
    noconformidad: "Hallazgo",
    observacion: "Hallazgo",
};
const ESTADO_HALLAZGO_MAP = {
    abierto: "En Proceso",
};
const ESTADO_PLAN_MAP = {
...
function mapHistoricalValue(value, map) {
...
function aliasesMatch(key, aliases) {
...
function hasColumn(rows, aliases) {
...
function parseDate(value) {
    ...
    if (human)
        return buildDate(Number(human[3]), months[human[2] ?? ""] ?? 0, Number(human[1]));
...
function buildParsedCases(payload, issues) {
    ...
    payload.rows.forEach((row, index) => {
        ...
        if (!codigo)
            addIssue(issues, requiredIssue(rowNumber, "Código", codigo));
        if (!tipo)
            addIssue(issues, requiredIssue(rowNumber, "Tipo", tipo));
        // El título pasó a ser opcional; la descripción es la que no puede faltar,
        // porque es de donde sale el encabezado del caso.
        if (!descripcion)
            addIssue(issues, requiredIssue(rowNumber, "Descripción", descripcion));
        if (!estado)
            addIssue(issues, requiredIssue(rowNumber, "Estado", estado));
        if (!fechaRaw) {
        ...
        if (codigo.length > 30)
            addIssue(issues, maxLengthIssue(rowNumber, "Código", codigo, 30));
        if (titulo && titulo.length > 200)
            addIssue(issues, maxLengthIssue(rowNumber, "Título", titulo, 200));
        if (reportante && reportante.length > 150)
            addIssue(issues, maxLengthIssue(rowNumber, "Reportante", reportante, 150));
...
function requiredIssue(row, field, value) {
    return {
        row,
        field,
        severity: "error",
...
function maxLengthIssue(row, field, value, max) {
    return {
        row,
        field,
        severity: "error",
...
function parsePlan(row, rowNumber, fallbackDate, caseState, caseArea, issues) {
    if (!planHasData(row))
        return null;
    ...
    if (!descripcion)
        addIssue(issues, requiredIssue(rowNumber, "Descripción Plan", descripcion ?? ""));
    if (!responsable)
        addIssue(issues, requiredIssue(rowNumber, "Responsable Plan", responsable ?? ""));
    if (!area)
        addIssue(issues, requiredIssue(rowNumber, "Área Plan", area ?? ""));
    if (fechaRaw && !fecha) {
        addIssue(issues, {
            row: rowNumber,
            field: "Fecha Plan",
    ...
    if (codigo && codigo.length > 50)
        addIssue(issues, maxLengthIssue(rowNumber, "Código Plan", codigo, 50));
    return {
        row: rowNumber,
        codigo,
        descripcion,
...
async function getBuildContext(parsed) {
    ...
    return {
        catalogos: catalogos.flatMap((catalogo) => catalogo.catalogo_detalle.map((detalle) => ({
...
function resolveCatalog(ctx, catalogName, value, row, field, issues) {
    const found = ctx.catalogos.find((item) => normalizeText(item.catalogo) === normalizeText(catalogName) &&
 ...
 */
function resolveCatalogOrDefault(ctx, catalogName, value, fallback, row, field, issues) {
    if (!value)
        return fallback;
    const found = ctx.catalogos.find((item) => normalizeText(item.catalogo) === normalizeText(catalogName) &&
        (normalizeText(item.nombre) === normalizeText(value) || (item.codigo ? normalizeText(item.codigo) === normalizeText(value) : false)));
    if (!found) {
        addIssue(issues, {
            ...
            message: CATALOGOS_AMPLIABLES.includes(catalogName)
...
function resolveRequiredDefault(ctx, catalogName, value, issues) {
    const found = ctx.catalogos.find((item) => normalizeText(item.catalogo) === normalizeText(catalogName) && normalizeText(item.nombre) === normalizeText(value));
    if (!found) {
        addIssue(issues, {
            row: 1,
            field: catalogName,
...
function resolveArea(ctx, value, row, field, issues) {
    if (!value)
        return null;
...
function resolveUser(ctx, value) {
    if (!value)
        return null;
    return (ctx.usuarios.find((usuario) => normalizeText(usuario.nombre) === normalizeText(value) ||
        normalizeText(usuario.correo) === normalizeText(value) ||
        normalizeText(usuario.codigo_usuario) === normalizeText(value)) ?? null);
}
/**
 ...
 */
const CATALOGOS_AMPLIABLES = ["Procedencia", "Tipo SOP", "Subtipo SOP"];
/**
 ...
 */
async function crearReferenciasFaltantes(payload) {
    const parsed = buildParsedCases(payload, []);
    const ctx = await getBuildContext(parsed);
    const creadas = { usuarios: 0, areas: 0, catalogos: 0 };
    const nombresUsuario = new Set();
    const nombresArea = new Set();
    const valoresCatalogo = new Map();
    const anotarArea = (valor) => {
        if (valor?.trim() && !resolveArea(ctx, valor, 0, "", []))
            nombresArea.add(valor.trim());
    };
    const anotarUsuario = (valor) => {
        if (valor?.trim() && !isAnonymousReporter(valor) && !resolveUser(ctx, valor))
            nombresUsuario.add(valor.trim());
    };
    for (const item of parsed) {
        ...
        for (const catalogo of CATALOGOS_AMPLIABLES) {
            ...
            if (!valor?.trim())
                continue;
            const existe = ctx.catalogos.some((detalle) => normalizeText(detalle.catalogo) === normalizeText(catalogo) && normalizeText(detalle.nombre) === normalizeText(valor));
            if (existe)
                continue;
            if (!valoresCatalogo.has(catalogo))
                valoresCatalogo.set(catalogo, new Set());
    ...
    if (nombresArea.size === 0 && nombresUsuario.size === 0 && valoresCatalogo.size === 0)
        return creadas;
 ...
 */
function traducirErrorDeEscritura(error) {
    const mensaje = error instanceof Error ? error.message : String(error);
    if (/too long for the column/i.test(mensaje)) {
        return new Error("La base de datos rechazó un valor por ser demasiado largo para su columna. " +
    ...
    if (/Transaction (already closed|not found)|timed out/i.test(mensaje)) {
        return new Error("La importación superó el tiempo máximo de la transacción. Reintente; si el archivo es muy grande, divídalo en dos partes.");
    }
...
function buildPreparedCases(parsed, ctx, issues) {
    ...
    for (const item of parsed) {
        if (!item.codigo || !item.tipo || !item.descripcion || !item.estado || !item.fecha)
            continue;
        ...
        const riesgo = item.riesgo && !EMPTY_RISK_VALUES.has(normalizeText(item.riesgo))
        ...
        if (!tipoReporte || !estadoHallazgo || !procedencia || !tipoSop || !defaultTipoCaso)
            continue;
        prepared.push({
            row: item.row,
            codigo: item.codigo,
...
function resolveUbicacionDesdeTitulo(ctx, titulo) {
    if (!titulo)
        return null;
    const tail = titulo.split("·").pop()?.trim();
    if (!tail || normalizeText(tail) === normalizeText(titulo))
        return null;
    return (ctx.catalogos.find((item) => normalizeText(item.catalogo) === normalizeText("Lugar Específico") && normalizeText(item.nombre) === normalizeText(tail)) ?? null);
}
function buildPreparedPlans(item, ctx, issues) {
    const result = [];
    item.plans.forEach((plan) => {
        if (!plan.descripcion || !plan.fecha || !plan.responsable)
            return;
        ...
        const estado = resolveCatalog(ctx, "Estado Plan de acción", mapHistoricalValue(plan.estado || DEFAULTS.estadoPlan, ESTADO_PLAN_MAP), plan.row, "Estado Plan", issues);
...
async function buildPreview(payload) {
    ...
    const duplicateRows = new Set(issues
    ...
    const cases = parsed.map((item) => ({
        ...
        status: errorRows.has(item.row) ? "error" : duplicateRows.has(item.row) ? "skipped" : "valid",
    }));
 ...
 */
async function createImportedCases(tx, items, actorId) {
    if (items.length === 0)
        return { casos: 0, eventos: 0, planes: 0 };
    ...
    const pedidos = items
        ...
        codigosExistentes: item.plans.map((plan) => plan.codigo).filter((codigo) => Boolean(codigo)),
    }))
    ...
    for (const lote of enLotes(items, INSERT_CHUNK)) {
        ...
        const idDe = (item) => {
            ...
            if (id == null)
                throw new Error(`No se pudo recuperar el caso ${item.codigo} recién creado`);
            return id;
        };
        ...
        await tx.evento_caso.createMany({
            data: lote.map((item, i) => {
                ...
                if (!evento)
                    throw new Error(`No se pudo recuperar el evento del caso ${item.codigo}`);
                return {
        ...
        const planesDelLote = lote.flatMap((item) => {
            ...
            return item.plans.map((plan) => {
                ...
                if (!codigoPlan)
                    throw new Error(`No se pudo generar un código de plan para el caso ${item.codigo}`);
                return {
                    id_caso: idDe(item),
    ...
    if (maxImportado > 0)
        await advanceSequenceAtLeast(tx, SEQ_CASOS_SOP, maxImportado);
    return { casos: items.length, eventos: items.length, planes };
}
export class ImportacionService {
    static async validar(payload) {
        const { preview } = await buildPreview(payload);
        return preview;
    }
    static async importar(payload, actorId, ip) {
