...
export class ImportacionController {
    ...
    static async importar(req, res) {
        try {
            if (!req.user?.id_usuario)
                return res.status(401).json(ApiResponse.error("Token inválido"));
            const payload = parsePayload(req.body);
    ...
    static async validarContingencias(req, res) {
        try {
            const payload = parsePayload(req.body);
    ...
    static async importarContingencias(req, res) {
        try {
            if (!req.user?.id_usuario)
                return res.status(401).json(ApiResponse.error("Token inválido"));
            const payload = parsePayload(req.body);
    ...
    static async validarMonitoreo(req, res) {
        try {
            const payload = parsePayload(req.body);
    ...
    static async importarMonitoreo(req, res) {
        try {
            if (!req.user?.id_usuario)
                return res.status(401).json(ApiResponse.error("Token inválido"));
            const payload = parsePayload(req.body);
